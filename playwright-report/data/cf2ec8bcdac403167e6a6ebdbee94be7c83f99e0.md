...
- Name: cart.spec.ts >> TC_009 - Cart page should show selected products @cart @regression
- Location: tests\cart.spec.ts:81:5
...
Call log:
  - waiting for locator('[data-test ="add-to-cart-${id}"]')
...
```ts
  ...
  12 |   } 
  13 |  
  14 |   async verifyProductsPageIsVisible(): Promise<void> { 
  15 |     await expect(this.page).toHaveURL(/inventory/); 
  16 |     await expect(this.page.locator('.inventory_list')).toBeVisible(); 
  17 | 
  18 |   } 
  19 |  
  20 |   
  21 |   async addProductToCart(productName: string): Promise<void> { 
  22 |     const id = productName.toLowerCase().replace(/ /g, '-');
  23 | 
> 24 |     await this.page.locator('[data-test ="add-to-cart-${id}"]').click();
     |                                                                 ^ Error: locator.click: Test timeout of 30000ms exceeded.
  25 |   }
  26 | 
  27 | async removeProductFromCart(productName: string): Promise<void> { 
  28 |     const id = productName.toLowerCase().replace(/ /g, '-');
  29 | 
  30 |     await this.page.locator('[data-test ="remove-${id}"]').click();
  31 |   }
  32 | 
  33 |   async goToCart(): Promise<void> {
  34 |     await this.cartLink.click();
  35 |   }
  36 | 
  37 |   async verifyCartCount(expectedCount: number): Promise<void> {
  38 |     
  39 |     if (expectedCount === 0) {
  40 |         await expect(this.cartBadge).toHaveCount(0); 
  41 |    } else{
  42 |     await expect(this.cartBadge).toHaveText(expectedCount.toString());
  43 |    }
  44 |      
  45 | }
  46 | }
